...
\hypersetup{%
	...
	pdftitle={Rapport TP3 : Classification},%
	pdfsubject={Classification},%
	pdfcreator={Antoine BOUCHER},%
	pdfproducer={LaTeX},%
	breaklinks=true,
	pdfkeywords={Classification, TP3},%
}%
...
\begin{frame}{The integral form of the Boltzmann equation}
    ...
    \fbox{\parbox{0.7\textwidth}{\centering\textbf{Problem: the solution depends on its own integral!} \\
   	$\Rdsh$ Let's introduce a numerical recursive MC scheme!}}
\end{frame}
...
\begin{frame}{2D Transparent medium}
	When $\sigma_t = \sigma_s = 0$, the explicit solution is given by: 
	\begin{equation*}
...
\begin{frame}{2D Absorption-scattering medium}
	With $\sigma_s = 0.5$ and $\sigma_t = 1$: no exact solution is known.
	\begin{figure}[H]
		\centering
		\begin{animateinline}[autoplay,loop,controls=none]{3} 
			\multiframe{11}{i=0+1}{
...
\begin{frame}{Conclusion}
	...
	\begin{itemize}
		...
		\item multiple schemes (backward here for radiation/forward, analog/semi-analog/non-analog) depending on the application,
		\item immediate parallelization.
	\end{itemize}
	\vspace{1em}
	...
	\begin{itemize}
		...
		\item implement more boundary conditions,
	\end{itemize}
	\label{LastPage}
\end{frame}
...
\begin{frame}[plain]{Website}
	...
	\centering
	\fbox{\color{IBMblue}\bfseries\href{https://clement16a.github.io/website2/index.html}{Resolution of the linear Boltzmann equation by Monte Carlo method}}
\end{frame}
